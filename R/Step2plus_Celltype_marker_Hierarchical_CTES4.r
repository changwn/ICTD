...
Step2plus_Celltype_marker_Hierarchical_CTES4 <- function (tg_R1_lists, data_CORS_cancer, data.matrix, tg_R1_list_stat, cell_type_enrich_cut = 0.45, tg_R1_cut = 6, ext_cn = 0, CT_balance = 1, immune_cell_uni_table0_GS, IM_id_list) 
{
    ...
    BCV_stat_c <- BCV_ttest3(data_23_s, rounds = 100, slice0 = 4, maxrank0 = 30, msep_cut = 1e-04)
    ...
    dim_diff_new <- (dim_tt - length(CTES0)) * ((dim_tt - length(CTES0)) > 
        0) + ext_cn
    print(paste("new_version, dim_diff:", dim_diff_new, 
        sep = ""))
    if (length(CTES0) > 0) {
        ROUNDS <- 3
    ...
    while ((NN < dim_diff) & (max(bbb[, 3] > 0.1)) & (length(tg_ids_c) > 0)) {
        ...
        sss_c[intersect(names(sss_c), names(sss_b))] <- sss_b[intersect(names(sss_c), 
            names(sss_b))]
        tg_scores_c <- bbb[tg_ids_c, 3]
        for (i in 1:length(tg_ids_c)) {
            tg_scores_c[i] <- tg_scores_c[i] + sss_c[names(tg_list)[tg_ids_c[i]]]
    ...
    names(ccc) <- c("Selected_Bases", "First_step_leaf_nodes", 
        "Second_step_filtered_leaf_nodes", "Further_added_other_leaves")
    return(ccc)
}